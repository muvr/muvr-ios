...
class MKSensorDataThreedDPerformanceTests : XCTestCase {
    ...
    func testAppendPerformance() {
        var d = try! MKSensorData(dimension: 3, start: 0, samplesPerSecond: 1, samples: [1, 1, 1])
        let fiftySamples = [Float](count: 50 * 3, repeatedValue: 100)
        
        self.measureBlock {
            for _ in 0...100_000 {
                try! d.append(try! MKSensorData(dimension: 3, start: d.end + 1, samplesPerSecond: 1, samples: fiftySamples))
            }
        }
    }
